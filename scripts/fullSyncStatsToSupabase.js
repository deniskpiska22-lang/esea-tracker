...
const MATCH_ID = (() => {
  const inline = argv.find((item) => item.startsWith("--match="));
  if (inline) return inline.slice("--match=".length).trim();
  const index = argv.indexOf("--match");
  return index >= 0 ? String(argv[index + 1] || "").trim() : "";
})();
const RETRY_FAILED = argv.includes("--retry-failed");
const RETRY_404 = argv.includes("--retry-404");
const FAILED_RETRY_HOURS = Math.max(0, Number(process.env.STATS_FAILED_RETRY_HOURS || 24));
const NOT_FOUND_RETRY_DAYS = Math.max(0, Number(process.env.STATS_404_RETRY_DAYS || 7));
...
function compactStats(body, matchId) {
  ...
  for (const player of players) {
    const key = player.teamId || player.teamName || "unknown-team";
    if (!teamsByKey.has(key)) {
      const sourceTeam = maps
        .flatMap((map) => map.teams)
        .find((team) =>
          (player.teamId && team.teamId === player.teamId) ||
          (!player.teamId && player.teamName && team.teamName === player.teamName)
        );

      teamsByKey.set(key, {
        teamId: player.teamId || sourceTeam?.teamId || null,
        teamName: player.teamName || sourceTeam?.teamName || "Unknown",
        score: Number(sourceTeam?.score || 0),
        players: [],
      });
    }
  ...
  const teams = [...teamsByKey.values()].filter(
...
function shouldExcludeFailure(failure) {
  if (!failure || RETRY_FAILED) return false;

  const lastAttempt = new Date(failure.lastAttemptAt || 0).getTime();
  const ageMs = Number.isFinite(lastAttempt) ? Date.now() - lastAttempt : Infinity;

  if (failure.status === 404 || failure.permanent === true) {
    if (RETRY_404) return false;
    return ageMs < NOT_FOUND_RETRY_DAYS * 86400000;
  }

  return ageMs < FAILED_RETRY_HOURS * 3600000;
}

async function loadAllCandidates(failedState) {
  if (MATCH_ID) {
    const { data, error } = await supabase
      .from("matches")
      .select("id,map_scores,player_stats,stats_synced,finished_at,status")
      .eq("id", MATCH_ID)
      .maybeSingle();

    if (error) throw error;
    if (!data) throw new Error(`Match not found in Supabase: ${MATCH_ID}`);
    return [data];
  }
  ...
  while (true) {
    ...
    for (const match of rows) {
      if (!shouldExcludeFailure(failedState[match.id])) {
        result.push(match);
      }
    }
...
async function main() {
  ...
  // Ошибки больше не исключаются навсегда. Временные ошибки повторяются
  // после STATS_FAILED_RETRY_HOURS, а 404 — после STATS_404_RETRY_DAYS.
  // Флаги --retry-failed и --retry-404 принудительно снимают паузу.
  ...
  const queue = await loadAllCandidates(failedState);

  console.log(`Database without stats: ${databaseRemaining}`);
  console.log(`Failures remembered: ${Object.keys(failedState).length}`);
  if (MATCH_ID) console.log(`Target match: ${MATCH_ID}`);
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      ...
      while (!finished && attempt < MAX_ATTEMPTS) {
        ...
        try {
          ...
          // 404 записывается в failed-файл, но не блокируется навсегда.
          // Повтор произойдёт после STATS_404_RETRY_DAYS или с --retry-404.
  ...
  console.log(
    "Failed matches are retried automatically after cooldown. " +
      "Use --retry-failed, --retry-404, or --match <id> for an immediate retry."
  );
