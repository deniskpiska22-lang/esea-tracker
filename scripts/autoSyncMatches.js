...
const championshipName = new Map(
  CHAMPIONSHIPS.map((item) => [item.id, item.name])
);
...
const DISCOVERY_DAYS_AHEAD = Number(process.env.DISCOVERY_DAYS_AHEAD || 7);
const FINISHED_DAYS_BACK = Number(process.env.FINISHED_DAYS_BACK || 7);
...
function scoreFromInternal(match, faction, index) {
  return (
    faction?.match_score ??
    match?.results?.score?.[faction?.premade_team_id] ??
    match?.results?.score?.[faction?.id] ??
    match?.[`team${index + 1}_score`] ??
    0
  );
}
...
function internalToRow(match) {
  ...
  return {
    ...
    competition_name:
      championshipName.get(match.championship_id) ||
...
function publicApiToPatch(data) {
  ...
  return {
    ...
    finished_at:
      asIso(data.finished_at) || (finished ? new Date().toISOString() : undefined),
    team1_id: first.id,
    team1_name: first.name,
    team1_slug: first.slug,
    team1_logo: first.logo,
...
function insideDiscoveryWindow(row) {
  const value = row.finished_at || row.scheduled_at;
  ...
  if (FINISHED_STATUSES.has(String(row.status).toUpperCase())) {
    return time >= now - FINISHED_DAYS_BACK * 86400000 && time <= now + 3600000;
  }
  return time >= now - LIVE_LOOKBACK_HOURS * 3600000 &&
    time <= now + DISCOVERY_DAYS_AHEAD * 86400000;
}
...
async function discoverMatches() {
  ...
  const jobs = trackedTeams.flatMap((team) => [
    { team, status: "MATCH_STATUS_SCHEDULED", limit: 40 },
    { team, status: "MATCH_STATUS_FINISHED", limit: 10 },
  ]);
  ...
  await runPool(jobs, async (job) => {
    try {
      ...
      for (const match of Array.isArray(data.payload) ? data.payload : []) {
        ...
        if (row && insideDiscoveryWindow(row)) rows.set(row.id, row);
      }
    } catch (error) {
      console.warn(
        `Discovery failed for ${job.team.name} (${job.status}): ${error.message}`
      );
    }
  });
...
async function upsertOnlyChanged(rows) {
  if (!rows.length) return { insertedOrChanged: 0, unchanged: 0 };
  ...
  const changedRows = rows
    .filter((row) => changed(existingById.get(row.id), row))
    .map((row) => ({ ...row, updated_at: new Date().toISOString() }));

  for (let index = 0; index < changedRows.length; index += 200) {
    const { error } = await supabase
      .from("matches")
      .upsert(changedRows.slice(index, index + 200), { onConflict: "id" });
    if (error) throw error;
  }

  return {
    insertedOrChanged: changedRows.length,
...
async function refreshActive() {
  ...
  await runPool(candidates, async (match) => {
    try {
...
async function main() {
  ...
  if (mode === "full") {
    ...
    discovery = {
      ...
      ...(await upsertOnlyChanged(discovered)),
    };
  }
